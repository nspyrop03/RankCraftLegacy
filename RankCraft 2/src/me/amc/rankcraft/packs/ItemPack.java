...
public class ItemPack extends Pack {

	private PackYaml y;
	private ConfigHelper c;
	
	public ItemPack(PackLevel level) {
		super(PackType.Items, level);
		
		y = new PackYaml("items");
		c = MainCore.instance.config;
		
		switch(level) {
		case Bronze:
			this.setName("§6BronzeItemPack");
			this.setCost(c.bronzeItemPackCost);
			break;
		case Gold:
			this.setName("§eGoldItemPack");
			this.setCost(c.goldItemPackCost);
			break;
		case Silver:
			this.setName("§7SilverItemPack");
			this.setCost(c.silverItemPackCost);
			break;
		default:
	...
	public void onRightClickPack(PlayerInteractEvent e) {
		...
		//	p.sendMessage("§2From this ItemPack you got: ");
			p.sendMessage(MainCore.instance.language.getOpenPack("Item"));
			...
			//	p.sendMessage("§6- "+amt+" "+mat);
				p.sendMessage(MainCore.instance.language.getItemFromPack("Item", amt, ""+mat));
			}
		...
		//	p.sendMessage("§2From this ItemPack you got: ");
			...
			break;
		default:
			System.out.println("[Error] ItemPack list not initialized!!!");
			break;
